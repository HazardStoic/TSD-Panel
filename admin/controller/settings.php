...
elseif (route(2) == "currency-manager"):
  ...
  currency_hash text COLLATE utf8mb4_unicode_ci NOT NULL
      ...
      if (!file_exists($_SERVER["DOCUMENT_ROOT"] . "/currencies.json")) {
        ...
        $json_content = HTTP_REQUEST("https://ownsmmpanel.in/currencies.json", "", array(""), "GET", 0);
  ...
  if ($access):
    if (isset($_GET["action"])) {
      if ($_GET["action"] == "buy_addon" && $_GET["addon"] == "google_login") {
        ...
        if (!$_SESSION["upi"]) {
          $upi = "upi://pay?pa=ownsmmpanel@ybl&pn=OwnSMMPanel&am=350.00&mam=0&tn=Google%20Login%20Addon&tr=$txn_id";
          $_SESSION["upi"] = $upi;
        }
