...
if (empty($action)) {
    // Your existing code for listing users and pagination
} elseif ($action == "delete_zero_fund_users") {
    // PHP functionality to delete users with zero funds
    try {
        $conn->beginTransaction();
        $delete = $conn->prepare("DELETE FROM clients WHERE balance = 0");
        $delete->execute();
        $conn->commit();

        $_SESSION["client"]["data"]["success"] = 1;
        $_SESSION["client"]["data"]["successText"] = "Users with zero funds were successfully deleted.";
    } catch (Exception $e) {
        $conn->rollBack();
        $_SESSION["client"]["data"]["error"] = 1;
        $_SESSION["client"]["data"]["errorText"] = "Failed to delete users with zero funds.";
    }
    header("Location:".site_url("admin/clients"));
    exit();
}
// Your existing code for other actions
      if( $_GET["mode"] ):
        $mode = $_GET["mode"];
if($mode == "dark"):
$mode = "dark";
